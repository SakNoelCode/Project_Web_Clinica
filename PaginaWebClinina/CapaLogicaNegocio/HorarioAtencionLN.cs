...
namespace CapaLogicaNegocio
{
    public class HorarioAtencionLN
    {
        ...
        public List<HorarioAtencion> ListarHorario(Int32 IdEspecialidad)
        {
            try
            {
                return HorarioAtencionDAO.getInstance().ListarHorario(IdEspecialidad);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public bool Actualizar(HorarioAtencion obj)
        {
            try
            {
                return HorarioAtencionDAO.getInstance().EditarEstado(obj);
            }
            catch (Exception ex)
            {
                throw ex;
